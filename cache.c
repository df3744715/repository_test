	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
	...
	for (bindex=0,i=0; i<nsets; i++)
    {
		...
		for (j=0; j<assoc; j++)
		{
			...
			//////////////NRU/////////////
			blk->nru_bit = 1;			//set initial used_bit to "1"	
			/////////////DRRIP/////////////
			blk->RRPV = 7;		//set initial RRPV to "7"
...
cache_char2policy(char c)		/* replacement policy as a char */
{
	switch (c) {
	case 'l': return LRU;
	case 'n': return NRU;		//NRU
	case 'd': return DRRIP;		//DRRIP
	     ...
	     FILE *stream)		/* output stream */
{
	...
	fprintf(stream,
		...
		cp->name, cp->assoc,
		cp->policy == LRU ? "LRU"
		: cp->policy == NRU? "NRU"  	//NRU policy
		: cp->policy == DRRIP? "DRRIP"		//DRRIP policy
...
struct cache_blk_t *
NRU_replace(struct cache_set_t *set)
{
	struct cache_blk_t *blk, *repl=NULL;		//set initial repl to NULL
	blk = set->way_head;		//initial blk 
	while(blk != NULL)
	{
		if(blk->nru_bit == 0)
		{
			repl = NULL;		//replace not find
			blk = blk->way_next;		//move to next blk
		}
		else if(blk->nru_bit == 1)
		{
			repl = blk;			//repl find out
			repl->nru_bit = 0;
			blk = NULL;		//finish
		}
	}
	if(repl == NULL)
	{
		...
		{
			blk->nru_bit = 1;		//set all used_bit "0" to "1"
		}
		repl = set->way_head;		//repl at the head
		repl->nru_bit = 0;
	}
	return repl;
}
...
struct cache_blk_t *
SRRIP_replace(struct cache_set_t *set)		//insert "6"
{
	struct cache_blk_t *blk, *repl=NULL;
	blk = set->way_head;
	int choose = 0;
	while(blk!=NULL)	//must has at least one "7" in a set
	{
		if(blk->RRPV != 7)
		{
			blk = blk->way_next;
	...
	while(repl==NULL && choose==0)		//not find any "7"
	{
		for(blk=set->way_head ; blk ; blk=blk->way_next)
		{
			blk->RRPV++;
			if(blk->RRPV==7&&choose==0)
			{
				repl = blk;
				repl->RRPV--;
				choose = 1;
...
struct cache_blk_t *
SRRIP2_replace(struct cache_set_t *set)		//insert "7"
{
	struct cache_blk_t *blk, *repl=NULL;
	blk = set->way_head;
	int choose = 0;
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
	...
	switch (cp->policy) 		//cache policy
	{
		case LRU:  
		//////NRU policy///////
		case NRU:
			repl = NRU_replace(&cp->sets[set]);
		break;
		////DRRIP////
		case DRRIP:
		{
			////SRRIP////
			if(set%(cp->nsets/32) == 0)		//first set
			{
				cp->PSEL ++;
				repl = SRRIP_replace(&cp->sets[set]);		//insert "6"
			}
			////BRRIP////
			else if((set+1)%(cp->nsets/32) == 0)		//second set
			{
				cp->PSEL --;
				if(cp->BIPCTR == 0)	
					repl = SRRIP_replace(&cp->sets[set]);		////0:2^M-2(6)
				else   
					repl = SRRIP2_replace(&cp->sets[set]);		////1~31:2^M-1(7)
			}
			////Follower sets ues DRRIP////	  
			else
			{ 
				if(cp->PSEL < 512)     //PSEL MSB=0 choose SRRIP
					repl = SRRIP_replace(&cp->sets[set]);		        
				else                 //PSEL MSB=1 choose BRRIP
				{
					if(cp->BIPCTR == 0)
						repl = SRRIP_replace(&cp->sets[set]);
					else
						repl = SRRIP2_replace(&cp->sets[set]);
				}	
			}
		}
		break;
